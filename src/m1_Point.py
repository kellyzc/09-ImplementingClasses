...
class Point(object):
    ...
    def move_to(self, x, y):
        self.distance_travelled = self.distance_travelled + self.get_distance_from(Point(x, y))
        self.x = x
        self.y = y
        self.number_of_moves = self.number_of_moves + 1

    def move_by(self, dx, dy):
        self.distance_travelled = self.distance_travelled + self.get_distance_from(Point(self.x + dx, self.y + dy))
        self.x = self.x + dx
        self.y = self.y + dy
        self.number_of_moves = self.number_of_moves + 1
    ...
    def get_distance_from_start(self):
        return self.get_distance_from(Point(self.starting_x, self.starting_y))

    def get_distance_travelled(self):
        return self.distance_travelled

    def closer_to(self, p2, p3):
        dist2 = self.get_distance_from(p2)
        dist3 = self.get_distance_from(p3)
        if dist2 <= dist3:
            return p2
        else:
            return p3
    ...
    def halfway_to(self, p2):
        return Point((self.x + (p2.x - self.x) / 2), (self.y + (p2.y - self.y) / 2))
...
def run_test_get_distance_traveled():
    ...
    print()
    print('Expected p1 has traveled 9.0')
    print('Actual:', p1.get_distance_travelled())
    ...
    print()
    print('Expected p2 has now traveled', 101 * 50.0)
    print('Actual:', p2.get_distance_travelled())
    print('Expected p3 has now traveled', 101 * 50.0)
    print('Actual:', p3.get_distance_travelled())
    print('Expected p4 has now traveled 100.0')
    print('Actual:', p4.get_distance_travelled())
